...
use resource::global::{GameAudio, GameTextures};
...
fn main() {
    App::new()
        .add_startup_system(setup_system.in_base_set(StartupSet::PreStartup))
        .add_startup_system(background_spawn_system)
        .add_system(monitor.run_if(on_timer(Duration::from_secs(2))))
...
fn setup_system(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    // add GameAudio resource
    let game_audio = GameAudio {
        player_fire: asset_server.load("Bonus/sfx_laser1.ogg"),
        enemy_fire: asset_server.load("Bonus/sfx_laser2.ogg"),
    };
