use crate::{
    ...
    resource::global::{GameAudio, GameTextures},
...
impl Plugin for EnemyPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(FormationMaker::default())
            .add_system(enemy_spawn_system.run_if(on_timer(Duration::from_millis(800))))
            .add_system(enemy_fire_system.run_if(|| thread_rng().gen_bool(1. / 60.)))
...
pub fn enemy_fire_system(
    ...
    for enemy_tf in enemy_query.iter() {
        ...
        let mut spawn_laser = |muzzle_offset| {
            audio.play(game_audio.enemy_fire.clone());
            commands
                .spawn(SpriteBundle {
